    ...
    static GString create(String resourceName, String target, Type type = Type.DEFAULT) {
        switch (type) {
            case Type.STYLE:
                ...
                def pattern = /(@${resourceName}\/${target}[\s!"#\$%&'()\*\+\-\,\\/:;<=>?@\[\\\]^`{|}~])|(R\.${resourceName}\.${toCamelCaseWithUnderscore(target)})|(${target}\.)|(parent="${target}")/
                return pattern
            case Type.DRAWABLE:
                // Considered 9patch
                target -= ".9"
                def pattern = /(@${resourceName}\/${target}[\s!"#\$%&'()\*\+\-\,\\\/:;<=>?@\[\\\]^`{|}~])|(R\.${resourceName}\.${target})/
                return pattern
            case Type.LAYOUT:
                // Considered data binding
                def pattern = /(@${resourceName}\/${target}[\s!"#\$%&'()\*\+\-\,\\\/:;<=>?@\[\\\]`{|}~])|(R\.${resourceName}\.${target})|(${toCamelCase(target)}Binding)/
                return pattern
            default:
                def pattern = /(@${resourceName}\/${target}[\s!"#\$%&'()\*\+\-\,\\\/:;<=>?@\[\\\]`{|}~])|(R\.${resourceName}\.${target})/
                return pattern
